...
**ENTSCHIEDEN UND UMGESETZT** in `20260730000000_employee_read_via_assignments.sql`: die Employee-**SELECT**-Policies auf `jobs`, `job_comments`, `job_photos` und `storage.objects` wurden um `or public.is_assigned_to_job(...)` erweitert. Keine einzige Schreibberechtigung geändert. 28 SQL-Fälle in `supabase/tests/employee_read_via_assignments.test.sql`.

> **Korrektur (2026-09-04).** Dieser Absatz zählte bis dahin auch die RPC `get_unread_comment_job_ids()` zu den erweiterten Objekten. Das war falsch — ein Rest aus einem früheren Entwurf: die Migration enthält keine Definition dieser Funktion, und ihr Abschnitt 5 heißt ausdrücklich „BEWUSST NICHT GEAENDERT: get_unread_comment_job_ids()". Die Rücknahme ist in diesem Dokument unter [„2. Ungelesen-Kennzeichnung: RPC-Erweiterung zurueckgenommen"](#2-ungelesen-kennzeichnung-rpc-erweiterung-zurueckgenommen) begründet. Der Satz hat realen Schaden angerichtet: `20260826000001` berief sich auf ihn, um die RPC erneut nicht anzufassen — dadurch blieb der Ungelesen-Punkt für sekundär Zugewiesene bis `20260904000000_unread_comments_via_assignments` aus.
