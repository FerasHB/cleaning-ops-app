...
// Nimmt access_token/refresh_token aus dem Link entgegen, baut daraus eine
// temporäre Recovery-Session auf und lässt den Nutzer ein neues Passwort
// setzen. Zustände: checking → ready → success, oder invalid bei
// ungültigem/abgelaufenem Link.
...
// Tokens landen je nach Supabase-Konfiguration entweder im Hash-Fragment
// (#access_token=...) oder als Query-Parameter (?access_token=...).
function extractParams(url: string): Record<string, string> {
  const hashIndex = url.indexOf("#");
  const queryIndex = url.indexOf("?");
  const splitIndex =
    hashIndex !== -1 ? hashIndex : queryIndex !== -1 ? queryIndex : -1;
...
export default function ResetPasswordScreen() {
  ...
  const handleIncomingUrl = async (url: string | null) => {
    ...
    // Kein Token im Link gefunden — evtl. besteht bereits eine
    // Recovery-Session (z.B. erneuter Aufruf des Effekts).
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setFormError("Ein unbekannter Fehler ist aufgetreten. Bitte versuche es erneut.");
    } finally {
      setSubmitting(false);
    }
  };
